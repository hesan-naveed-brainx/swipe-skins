...
class JsonCardBuilder extends Controller
{
    public function makeJsonFile(Request $request)
    {
        $color= str_replace('"', '', $request->color);
        $baseImage = str_replace('"', '', $request->baseImage); //Black and white image for json card
		$transparentImage = str_replace('"', '', $request->cardDesign); // border, card logo and text section
		$coloredDesignImage = str_replace('"', '', $request->cardDesignColored); // full card with color
		$customLogo = str_replace('"', '', $request->customLogo); //uploaded logo in original form
		
		//Save transparent base64 png to png image 
		$transparentFileName = $this->convertBaseToPng('card', $transparentImage);

		//Save Full colored card base64 png to png image 
		$coloredCardFileName = $this->convertBaseToPng('colorCard', $coloredDesignImage);
		
		$customLogoFileName = $customLogo;

		if (strpos($customLogo, ';base64,') !== false) {
			$customLogoFileName = $this->convertBaseToPng('logo', $customLogo);
		}
		
        $jsonCard = json_decode(file_get_contents("jsonCards/Metal_card_".$color.".json"));
		$jsonCard->images[1]->url = $baseImage;
		$content = json_encode($jsonCard, JSON_UNESCAPED_SLASHES);
		
		//Make new file of JSON and return file address to SHOPIFY
		$unique_url = 'assets/'.uniqid().time().$color.".json";

		$fp = fopen($unique_url,"w") or die("Unable to open file!");
		...
		if( $fp == false ){
			...
			fwrite($fp, $content);
			fclose($fp);
			return array($unique_url, $transparentFileName, $customLogoFileName, $coloredCardFileName );
		}
    }
	
	public function convertBaseToPng($type, $baseCodeImage)
	{

		$image_parts = explode(";base64,", $baseCodeImage);
		$image_type_aux = explode("image/", $image_parts[0]);
		$image_type = $image_type_aux[1];
		$image_base64 = base64_decode($image_parts[1]);
		$transparentFileName = 'cardStyles/' . $type . uniqid() . '.png';
		file_put_contents($transparentFileName, $image_base64);
		return $transparentFileName;
	}

    public function getJsonFile(Request $request)
    {
        $jsonFileName = str_replace('"', '', $request->jsonFile);
        $jsonResult = file_get_contents( public_path($jsonFileName));
		return $jsonResult;
    }
	...
	public function convertImage(Request $request){
		...
		if ($request->hasFile('file')) {
			...
			// file_put_contents($fileNameToStore, $file); 
			$file->move('cardStyles/', 'uploaded_logo_'.$fileNameToStore);
			$filePath = 'cardStyles/uploaded_logo_'.$fileNameToStore;


			// If image is png. Then convert it to JPG image
			...
			$pot = new Potracio();
			$pot->loadImageFromFile($filePath);
			$pot->process();
			$svg = $pot->getSVG(3);
